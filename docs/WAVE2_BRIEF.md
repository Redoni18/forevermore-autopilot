...
8. **Hosting = one Linux VPS** (Hetzner CPX31-class: 4 vCPU / 8 GB / ~€16.5/mo, Ubuntu LTS, TZ Europe/Tirane): Postgres (same schema, same :5433 parity), tick + Telegram poller + Station as systemd units, a read-only clone of the platform repo as `FOREVERMORE_ROOT` (after the §3.12 kit move it's needed only for template thumbnails and capture sources), Cloudflare Tunnel + Cloudflare Access (my email only) in front of the Station (which keeps its 127.0.0.1 bind and no-auth design), R2 for publish media + nightly `pg_dump` backups. The Mac becomes a dev machine only; its launchd tick gets unloaded at cutover — never two tickers against one DB. The staged GitHub-Actions scheduler (`ops/github/autopilot.yml`) and RUNBOOK's Mac→GHA section are superseded by this decision; leave them but don't wire them.
9. **Renderer portability is a first-class task, not a footnote.** Poster rendering shells out to headless Brave on macOS and video runs through the marketing kit's Remotion studio (`marketing/04-assets/render.mjs`, `marketing/05-video-studio` — both come in-repo with §3.12). On Linux: browser path must be configurable (chrome-headless-shell / chromium), Remotion needs `--gl=swangle` on CPU-only boxes, and renders will be slower — that's fine at our volume. Prove render parity on the VPS (same item renders, plausible bytes, lint passes) before cutover.
...
12. **The marketing kit moves into this repo (Phase 2 pre-work).** Verified 2026-07-13: `marketing/` is **untracked** in the platform repo — the brand guide, poster templates, `render.mjs`, and the Remotion video studio exist only as loose files on the Mac. A `git clean -fd` in the platform repo would delete them, and a platform clone on the VPS would silently miss them. Move `marketing/` wholesale into this repo (e.g. as `kit/`), gitignore its `node_modules` (~1 GB, reinstallable) and rendered outputs, and re-point every consumer (`src/config.mjs` resolution, `brain/assemble.mjs`, lint config, render/video adapters, showcase gates). This also internalizes the `renderOne`/`renderJobs` coupling. Afterward the `FOREVERMORE_ROOT` seam shrinks to: template thumbnails (their upstream source stays the platform's thumbnail pipeline) and dev-server world captures for the library — nothing else. Leave a pointer README at the platform's `marketing/` location; coordinate with me before deleting anything there.
...
**Pre-work, on the Mac, before any provisioning:** (a) execute the §3.12 marketing-kit move and prove an end-to-end render from the new in-repo location (same item re-renders, lint passes, tests green); (b) extend `make doctor` with a contract check — today it only probes the Brave/Remotion binaries; make it verify every remaining path/interface consumed from `FOREVERMORE_ROOT` (per ADR-001) plus the kit's own internal contract (poster templates, studio comps, brand-guide sections, render exports), so drift fails loudly — and reaches me as a Telegram alert once Phase 1 is live.
...
- Render depends on the marketing kit's `render.mjs` exports (`renderOne`/`renderJobs`) — sitting in the platform tree until the §3.12 move brings the kit in-repo; coordinate, never fork.

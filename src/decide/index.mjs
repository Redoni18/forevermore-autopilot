// Shared decision core (extracted from review/lib/store.mjs at Phase 1).
//
...
// re-export shim, so app.mjs and the existing tests see zero churn): the only
// edit is the relative import of the state machine (`../state/machine.mjs`
// instead of `../../src/state/machine.mjs`). Assets remain the review layer's
// concern (streamed off disk by app.mjs); this module only orchestrates the
// item/decision records against the {@link import('../types.mjs').Store}.

import { regenNext } from '../state/machine.mjs';
 ...
 */
export async function autoSkipSiblings(store, candidateGroup, chosenId, via) {
  const skipped = [];
 ...
 * @param {Object} args @param {import('../types.mjs').Store} args.store
 */
